...
- Saved figure outputs (PNG) of the state transitions
...
| File | Description |
| :--- | :--- |
| **`data/`** | Input data folder for dFC analysis.
...
1. Download the dataset from [Figshare](https://doi.org/10.6084/m9.figshare.31986894) and place the `Dynamic_functional_connectivity_data.mat` file into the `data/` folder.
   ...
   **Expected directory structure:**
   ...
   .
   ├── data/
   │   └── Dynamic_functional_connectivity_data.mat  # Input data for dFC analysis
   ├── Dynamic_functional_connectivity_analysis.m 
   └── ...
   ```
...
The demo dataset is provided as `Dynamic_functional_connectivity_data.mat`. Running `Dynamic_functional_connectivity_analysis.m` on this dataset displays the dynamic phase matrix and dynamic transitions of connectivity states in MATLAB.
...
To run the software on user data, prepare the input data in MATLAB `.mat` format with the same variable structure as `Dynamic_functional_connectivity_data.mat`, place the file into the `data/` folder, and modify the data-loading section of `Dynamic_functional_connectivity_analysis.m` if the file name or variable names are different from the demo dataset.
